...
class certificate {
     ...
     *      If you want to display all linkedincerts on a page set this to 0.
     */
    const linkedincert_PER_PAGE = '50';

    /**
     * Handles setting the protection field for the linkedincert
     *
     * @param \stdClass $data
    ...
    /**
     * Handles uploading an image for the linkedincert module.
     *
    ...
    public static function upload_files($draftitemid, $contextid, $filearea = 'image') {
        ...
        file_save_draft_area_files($draftitemid, $contextid, 'mod_linkedincert', $filearea, 0);
    }
    ...
    /**
     * Returns a list of issued linkedincerts.
     *
     ...
     */
    public static function get_issues($linkedincertid, $groupmode, $cm, $limitfrom, $limitnum, $sort = '') {
        global $DB;
        ...
        $sql = "SELECT $ufields, ci.id as issueid, ci.code, ci.timecreated
                  FROM {user} u
            INNER JOIN {linkedincert_issues} ci
                    ON u.id = ci.userid
                 WHERE u.deleted = 0
    ...
    /**
     * Returns the total number of issues for a given linkedincert.
     *
     * @param int $linkedincertid
     * @param \stdClass $cm the course module
     ...
     */
    public static function get_number_of_issues($linkedincertid, $cm, $groupmode) {
        global $DB;
    ...
    public static function get_conditional_issues_sql($cm, $groupmode) {
        ...
        // Get all users that can manage this linkedincert to exclude them from the report.
        $context = \context_module::instance($cm->id);
        $conditionssql = '';
        $conditionsparams = array();

        // Get all users that can manage this certificate to exclude them from the report.
        $certmanagers = array_keys(get_users_by_capability($context, 'mod/linkedincert:manage', 'u.id'));
    ...
    public static function issue_certificate($certificateid, $userid) {
        ...
        return $DB->insert_record('linkedincert_issues', $issue);
    }
    ...
    public static function generate_code() {
        ...
        while (!$uniquecodefound) {
            if (!$DB->record_exists('linkedincert_issues', array('code' => $code))) {
